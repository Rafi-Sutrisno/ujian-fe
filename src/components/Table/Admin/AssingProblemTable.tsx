...
interface AssignProblemTableProps {
  id: string
  onSuccess?: () => void
...
const columns: readonly Column[] = [{ id: 'title', label: 'Title', minWidth: 150 }]
...
const AssignProblemTable = forwardRef<AssignProblemTableRef, AssignProblemTableProps>(
  ({ id, onSuccess, onError }, ref) => {
    ...
    const handleCreateMany = async () => {
      ...
      const payload = selectedRows.map(row => ({
        problem_id: row.id,
        exam_id: id // from props
      }))

      try {
        const data = await fetchWithAuth(`/api/exam_problem/create_many`, payload, 'POST')
...
export default AssignProblemTable
