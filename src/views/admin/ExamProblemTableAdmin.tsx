...
const ExamProblemTableAdmin: React.FC<ProblemTableProps> = ({ exam_id }) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: 'Problem removed from exam successfully!',
        severity: 'success'
      })
  ...
  const handleSave = () => {
    assignProblemTableRef.current?.handleCreateMany()
  }
  ...
  return (
    <Card sx={{ mt: 4 }}>
      <CardContent>
        <TopSectionModal
          title='Problem List'
          buttonText='Assign Problem'
          modalContent={
            <AssignProblemTable ref={assignProblemTableRef} id={exam_id} onSuccess={onSuccess} onError={onError} />
          }
          onSave={handleSave}
        />
                          ...
                          return (
                            <TableCell key={column.id} align={column.align ?? 'left'}>
                              <Stack direction='row' spacing={2}>
                                <Link href={`/admin/problem/${row.problem_id}`} passHref>
...
export default ExamProblemTableAdmin
